	...
	float terrainHeight)
{
	...
	result = m_MiniMapBitmap->Initialize(device,hwnd, screenWidth, screenHeight, L"../Engine/data/colorm01.dds", 150, 150);
	if (!result)
	{
	...
	result = m_Border->Initialize(device, hwnd,screenWidth, screenHeight, L"../Engine/data/border01.dds", 154, 154);
	if (!result)
	{
	...
	result = m_Point->Initialize(device, hwnd, screenWidth, screenHeight, L"../Engine/data/point01.dds", 3, 3);
	if (!result)
	{
